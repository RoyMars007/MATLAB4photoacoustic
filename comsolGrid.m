classdef comsolGrid < handle
    ...
    methods
        function crid = comsolGrid(varargin)
            ...
            switch crid.dim
                case 1
                    ...
                    crid.kx_vec = crid.makeDim(crid.Nx, crid.dx);
                   
                    % define the scalar wavenumber based on the wavenumber components
                    crid.k = abs(crid.kx_vec);
                    
                    % define maximum supported frequency
                    crid.kx_max = max(abs(crid.kx_vec(:)));
                    crid.k_max  = crid.kx_max;
                    
                case 2
                    
                    % assign the grid parameters for the x and y spatial directions
                    crid.kx_vec = crid.makeDim(crid.Nx, crid.dx);
                    crid.ky_vec = crid.makeDim(crid.Ny, crid.dy);

                    % define the wavenumber based on the wavenumber components
                    % (using bsxfun saves memory by avoiding ndgrid)
                    crid.k = zeros(crid.Nx, crid.Ny);
                    crid.k = bsxfun(@plus, (reshape(crid.kx_vec, [], 1, 1).^2), crid.k);
                    crid.k = bsxfun(@plus, (reshape(crid.ky_vec, 1, [], 1).^2), crid.k);
                    crid.k = sqrt(crid.k);
                    
                    % define maximum supported frequency
                    crid.kx_max = max(abs(crid.kx_vec(:)));
                    crid.ky_max = max(abs(crid.ky_vec(:)));
                    crid.k_max  = min([crid.kx_max, crid.ky_max]); 
                ...
                case 3
                    ...
                    crid.kx_vec = crid.makeDim(crid.Nx, crid.dx);
                    crid.ky_vec = crid.makeDim(crid.Ny, crid.dy);
                    crid.kz_vec = crid.makeDim(crid.Nz, crid.dz);

                    % define the wavenumber based on the wavenumber components
                    % (using bsxfun saves memory by avoiding ndgrid)
                    crid.k = zeros(crid.Nx, crid.Ny, crid.Nz);
                    crid.k = bsxfun(@plus, (reshape(crid.kx_vec, [], 1, 1).^2), crid.k);
                    crid.k = bsxfun(@plus, (reshape(crid.ky_vec, 1, [], 1).^2), crid.k);
                    crid.k = bsxfun(@plus, (reshape(crid.kz_vec, 1, 1, []).^2), crid.k);
                    crid.k = sqrt(crid.k);                   
                   
                    % define maximum supported frequency
                    crid.kx_max = max(abs(crid.kx_vec(:)));
                    crid.ky_max = max(abs(crid.ky_vec(:)));
                    crid.kz_max = max(abs(crid.kz_vec(:)));
                    crid.k_max  = min([crid.kx_max, crid.ky_max, crid.kz_max]);  
